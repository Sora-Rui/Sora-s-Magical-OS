...
local function ensureMonitorName(state)
    if state.monitorName or not state.monitor or not peripheral or not peripheral.getName then
        return state.monitorName
    end

    local ok, name = pcall(peripheral.getName, state.monitor)
    if ok and name then
...
local function drawMonitorPromptOverlay(state, title)
    local monitorName = ensureMonitorName(state)
    if not state.monitor or not monitorName then
        return nil
    end
...
local function promptSaveUser(state, nativeTerm)
    ...
    local defaultUser = runtime.selectedCrewUserName(state)
    if defaultUser == "Kein Crew-Konto" then
        defaultUser = ""
    end
    local username, cancelled = promptField(state, nativeTerm, "Crew-Konto", instructions, "Benutzer", defaultUser)
    if cancelled then
        setPromptCancelled(state)
        return
    end
...
local function promptAssignRole(state, nativeTerm)
    ...
    if not user then
        runtime.setNotice(state, "Bitte erst Crew-Konto auswaehlen", 24)
        return
    end
    local ok, reason = runtime.assignRoleToUser(state, user.name, state.crew.selectedRole)
    if not ok then
...
local function promptRemoveRole(state, nativeTerm)
    ...
    local ok, reason = runtime.removeRoleFromUser(state, user.name, state.crew.selectedRole)
    if not ok then
...
local function promptDeleteUser(state, nativeTerm)
    ...
    local ok, reason = runtime.deleteCrewUser(state, user.name)
    if not ok then
...
local function handleAction(state, action, nativeTerm, context)
    ...
    if string.sub(action, 1, 12) == "select_user:" then
        runtime.selectCrewUser(state, string.sub(action, 13))
    elseif action == "select_prev_user" then
        runtime.cycleCrewUserSelection(state, -1)
    elseif action == "select_next_user" then
        runtime.cycleCrewUserSelection(state, 1)
    elseif action == "toggle_alarm" then
        runtime.toggleManualAlarm(state)
    elseif action == "audio_prev" then
        runtime.cycleAudioCue(state, -1)
    elseif action == "audio_next" then
        runtime.cycleAudioCue(state, 1)
    elseif action == "play_audio_cue" then
        runtime.playSelectedAudioCue(state)
    elseif action == "test_speaker" then
        runtime.testSpeaker(state)
...
local function main()
    ...
    while true do
        runtime.prepareDisplay(state, nativeTerm)
        render(state)
        runtime.renderDebugTerminal(state, nativeTerm)
        local event, p1, p2, p3 = os.pullEvent()
