...
local function promptRoleLogin(state, nativeTerm)
    beginPrompt(nativeTerm, "Crew-Login", {
        "Rolle: " .. runtime.selectedRoleLabel(state),
        "Operatorname und Rollencode eingeben.",
    })
    local defaultOperator = runtime.operatorLabel(state) ~= "Gast" and runtime.operatorLabel(state) or ""
    local operator = promptField("Operator", defaultOperator)
    local roleCode = promptField("Rollencode", "", true)
    runtime.loginRole(state, state.crew.selectedRole, operator, roleCode)
end

local function promptRoleCode(state, nativeTerm)
    beginPrompt(nativeTerm, "Rollencode aendern", {
        "Rolle: " .. runtime.selectedRoleLabel(state),
        "Neuen Rollencode zweimal eingeben.",
    })
    local first = promptField("Neuer Code", "", true)
    local second = promptField("Wiederholen", "", true)
    if first ~= second then
        runtime.setNotice(state, "Rollencode stimmt nicht ueberein", 24)
        runtime.log(state, "Rollencode-Aenderung fehlgeschlagen", "SEC")
        return
    end
    runtime.setRoleCode(state, state.crew.selectedRole, first)
end
...
local function handleAction(state, action, nativeTerm, context)
    ...
    elseif action == "set_role_code" then
        promptRoleCode(state, nativeTerm)
