...
function screens.crew(state)
    ...
    local manifestLines = runtime.crewManifestLines(state, 3)

    if ui.isLargeLayout() then
        ui.monitorFrame("Crew und Sicherheit", "Login, Rollenbereiche und Tablet-Freigabe", state)
        ...
        ui.metricPanel(61, top, 17, " Sicherheit ", securityText, securityColor, runtime.describeAssignment(state, "keySwitchSide"))

        ui.panel(4, top + 6, 74, 7, " Crew-Login und Bereiche ")
        ui.button(state, 8, top + 8, 12, "Pilot", "role_pilot", state.crew.selectedRole == "pilot")
        ui.button(state, 22, top + 8, 14, "Ingenieur", "role_engineer", state.crew.selectedRole == "engineer")
        ui.button(state, 38, top + 8, 16, "Alarmzentrale", "role_alarm", state.crew.selectedRole == "alarm")
        ui.button(state, 56, top + 8, 14, "Captain", "role_captain", state.crew.selectedRole == "captain")
        ui.button(state, 8, top + 10, 12, "Login", "login_role")
        ui.button(state, 22, top + 10, 12, "Logout", "logout_role")
        ui.button(state, 36, top + 10, 12, "PIN", "unlock_security")
        ui.button(state, 50, top + 10, 12, "Schalter", "map_key_switch")
        ui.writeAt(8, top + 12, runtime.selectedRoleHint(state), theme.muted, theme.panel)

        ui.panel(4, top + 14, 74, 5, " Captain-Verwaltung ")
        for index, line in ipairs(manifestLines) do
            ui.writeAt(8, top + 15 + index, line, theme.text, theme.panel)
        end
        ui.button(state, 56, top + 16, 10, "User", "save_user")
        ui.button(state, 68, top + 16, 8, "Rolle", "assign_selected_role")
        if runtime.isAlarmVisible(state) then
    ...
    ui.frame("Crew und Sicherheit", "B Home  |  Login auf Crew", state)
    ui.statusBar(state)
    ...
    ui.statusRow(12, "Sicherheit", securityText, securityColor)
    ui.statusRow(13, "Crew", tostring(runtime.crewUserCount(state)) .. " Konten", theme.text)
    ui.button(state, 4, height - 4, 10, "Pilot", "role_pilot", state.crew.selectedRole == "pilot")
    ui.button(state, 16, height - 4, 10, "Ing", "role_engineer", state.crew.selectedRole == "engineer")
    ui.button(state, 28, height - 4, 10, "Alarm", "role_alarm", state.crew.selectedRole == "alarm")
    ui.button(state, 40, height - 4, 10, "Capt", "role_captain", state.crew.selectedRole == "captain")
    ui.button(state, 4, height - 3, 12, "Login", "login_role")
    ui.button(state, 18, height - 3, 12, "Logout", "logout_role")
    ui.button(state, 32, height - 3, 12, "PIN", "unlock_security")
    ui.button(state, 4, height - 2, 12, "User", "save_user")
    ui.button(state, 18, height - 2, 12, "Rolle", "assign_selected_role")
    ui.writeAt(32, height - 2, runtime.selectedRoleHint(state), theme.muted, theme.accent)
    if runtime.isAlarmVisible(state) then
        ui.warningOverlay("ALARM", { "Crewpage zeigt Sicherheitszustand" }, true)
    end
end
