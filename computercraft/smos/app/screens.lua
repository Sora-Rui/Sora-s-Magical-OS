...
function screens.helm(state)
    ...
    if ui.isLargeLayout() then
        ...
        ui.metricPanel(64, top, 14, " Kritisch ", securityText, securityColor, state.helm.failsafe)
    ...
    ui.statusRow(11, "Kritisch", securityText, securityColor)
...
function screens.crew(state)
    ...
    local manifestLines = runtime.crewManifestLines(state, 2)
    ...
    if ui.isLargeLayout() then
        ...
        ui.metricPanel(61, top, 17, " Freigabe ", securityText, securityColor, runtime.securitySourceLabel(state))
        ...
        ui.button(state, 32, top + 10, 8, "PIN", "unlock_security")
        ui.button(state, 42, top + 10, 12, "Schalter", "map_key_switch")
        ...
        ui.button(state, 8, top + 18, 12, "User+", "save_user")
        ui.button(state, 22, top + 18, 12, "User-", "delete_user")
        ui.button(state, 36, top + 18, 12, "Rolle+", "assign_selected_role")
        ui.button(state, 50, top + 18, 12, "Rolle-", "remove_selected_role")
        if runtime.isAlarmVisible(state) then
    ...
    ui.statusRow(12, "Freigabe", securityText, securityColor)
    ...
    ui.button(state, 28, height - 4, 8, "PIN", "unlock_security")
    ui.button(state, 38, height - 4, 10, "Switch", "map_key_switch")
    ...
    ui.writeAt(4, height - 2, runtime.securitySummary(state), theme.muted, theme.accent)
    if runtime.isAlarmVisible(state) then
        ui.warningOverlay("ALARM", { "Crewpage zeigt Sicherheitszustand" }, true)
    end
end
