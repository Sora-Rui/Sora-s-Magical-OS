...
function screens.crew(state)
    ...
    if ui.isLargeLayout() then
        ...
        ui.button(state, 8, top + 8, 14, "Pilot", "role_pilot", state.crew.selectedRole == "pilot")
        ui.button(state, 24, top + 8, 16, "Ingenieur", "role_engineer", state.crew.selectedRole == "engineer")
        ui.button(state, 42, top + 8, 18, "Alarmzentrale", "role_alarm", state.crew.selectedRole == "alarm")
        ui.button(state, 62, top + 8, 12, "Login", "login_role")
        ui.button(state, 8, top + 10, 16, "Logout", "logout_role")
        ui.button(state, 27, top + 10, 16, "Rollen-Code", "set_role_code")
        ui.button(state, 46, top + 10, 12, "PIN", "unlock_security")
        ui.button(state, 60, top + 10, 14, "Schalter I/O", "map_key_switch")
        ui.writeAt(8, top + 12, "Gaeste sehen nur Home und Crew. Bereiche oeffnen sich erst nach Login.", theme.muted, theme.panel)
        if runtime.isAlarmVisible(state) then
    ...
    ui.statusRow(9, "Operator", runtime.operatorLabel(state), theme.text)
    ui.statusRow(10, "Rolle", runtime.roleLabel(state), theme.text)
    ui.statusRow(11, "Auswahl", runtime.selectedRoleLabel(state), theme.text)
    ui.statusRow(12, "Sicherheit", securityText, securityColor)
    ui.statusRow(13, "Code", runtime.roleCodePreview(state), theme.text)
    ui.button(state, 4, height - 3, 12, "Pilot", "role_pilot", state.crew.selectedRole == "pilot")
    ui.button(state, 18, height - 3, 12, "Ing", "role_engineer", state.crew.selectedRole == "engineer")
    ui.button(state, 32, height - 3, 12, "Alarm", "role_alarm", state.crew.selectedRole == "alarm")
    ui.button(state, 4, height - 2, 12, "Login", "login_role")
    ui.button(state, 18, height - 2, 12, "Logout", "logout_role")
    ui.button(state, 32, height - 2, 12, "Code", "set_role_code")
    if runtime.isAlarmVisible(state) then
        ui.warningOverlay("ALARM", { "Crewpage zeigt Sicherheitszustand" }, true)
    end
end
...
function screens.comms(state)
    ...
    if ui.isLargeLayout() then
        ...
        ui.writeAt(8, top + 8, "Pocket Computer: tablet.lua starten, Schiffsnamen setzen, Rolle waehlen.", theme.muted, theme.panel)
        ui.writeAt(8, top + 9, "Fuer Rollenbereiche braucht das Tablet den Rollencode, fuer kritische Befehle zusaetzlich die globale PIN.", theme.muted, theme.panel)
