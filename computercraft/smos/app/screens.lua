...
local function drawBridgeAlarmButton(state, x, y, width, height, active)
    local background = colors.red
    local textColor = active and colors.yellow or colors.white
    local buttonHeight = math.max(1, height or 1)
    local label = "Alarm"

    for row = 0, buttonHeight - 1 do
        ui.writeAt(x, y + row, string.rep(" ", width), textColor, background)
    end

    local labelY = y + math.floor((buttonHeight - 1) / 2)
    ui.writeAt(x + math.max(0, math.floor((width - #label) / 2)), labelY, label, textColor, background)
...
function screens.home(state)
    ...
    if ui.isLargeLayout() then
        ...
        drawBridgeAlarmButton(state, 49, 4, 12, 3, state.manualAlarm)
