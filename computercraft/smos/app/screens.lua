...
local function drawBridgeAlarmButton(state, x, y, width, active)
    ...
    local label = " Alarm "
    local rendered = label .. string.rep(" ", math.max(0, width - #label))

    ui.writeAt(x, y, rendered, textColor, background)
...
function screens.home(state)
    ...
    if ui.isLargeLayout() then
        ...
        drawBridgeAlarmButton(state, 68, top + 2, 9, state.manualAlarm)
