...
local function roleLabelsForUser(user)
    local labels = {}
    for _, roleName in ipairs({ "captain", "pilot", "engineer", "alarm" }) do
...
function runtime.canManageCrew(state)
    return state.crew.session and state.crew.session.role == "captain"
end
...
function runtime.assignRoleToUser(state, username, roleName)
    local definition = ROLE_DEFINITIONS[roleName]
    local _, user = findCrewUser(state, username)
    if not definition then
        return false, "Unbekannte Rolle"
    end
...
function runtime.crewManifestLines(state, maxRows)
    ...
    table.sort(keys, function(left, right)
        return tostring(state.crew.users[left].name or left) < tostring(state.crew.users[right].name or right)
    end)
    ...
    for index = 1, math.min(maxRows or #keys, #keys) do
        local user = state.crew.users[keys[index]]
        rows[#rows + 1] = user.name .. ": " .. roleLabelsForUser(user)
    end
...
function runtime.selectedRoleHint(state)
    ...
    return "Captain vergibt Rollen an Crew-Konten"
end
