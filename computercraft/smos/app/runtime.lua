...
local alarmNotes = {
    {
        siren = {
            { instrument = "bass", pitch = 2, volume = 1 },
            { instrument = "didgeridoo", pitch = 6, volume = 3 },
            { instrument = "bit", pitch = 14, volume = 1 },
        },
    },
    {
        siren = {
            { instrument = "bass", pitch = 3, volume = 1 },
            { instrument = "didgeridoo", pitch = 10, volume = 3 },
            { instrument = "bit", pitch = 18, volume = 1 },
        },
    },
    {
        siren = {
            { instrument = "bass", pitch = 4, volume = 1 },
            { instrument = "didgeridoo", pitch = 13, volume = 3 },
            { instrument = "bit", pitch = 21, volume = 1 },
        },
    },
    {
        siren = {
    ...
    {
        siren = {
            { instrument = "bass", pitch = 1, volume = 1 },
            { instrument = "didgeridoo", pitch = 4, volume = 3 },
            { instrument = "bit", pitch = 12, volume = 1 },
        },
    },
}
...
local function formatSpeakerResult(ok, result)
    if ok then
        if result == false then
            return "Speaker busy oder stumm"
        end

        return "Speaker spielt"
    end

    return "Speaker-Fehler: " .. tostring(result)
end

local function invokeSpeakerSequence(state, steps)
    local unpackArgs = table.unpack or unpack
    local failures = {}

    for _, step in ipairs(steps or {}) do
        local ok, result = speakerInvoke(state, step.method, unpackArgs(step.args or {}))
        if ok and result ~= false then
            return true, step.label or step.method, step.successMessage or "Speaker spielt"
        end

        failures[#failures + 1] = (step.label or step.method) .. ": " .. formatSpeakerResult(ok, result)
    end

    return false, steps and steps[1] and (steps[1].label or steps[1].method) or "-", table.concat(failures, " | ")
end

local recordAudioResult
...
local function playAlarmTone(state)
    ...
    local played = false
    local method = "-"
    local message = "Alarmton blockiert oder stumm"

    if note and note.siren then
        local failures = {}
        for _, tone in ipairs(note.siren) do
            local ok, result = speakerInvoke(state, "playNote", tone.instrument, tone.volume or 3, tone.pitch)
            if ok and result ~= false then
                played = true
            else
                failures[#failures + 1] = tone.instrument .. ": " .. formatSpeakerResult(ok, result)
            end
        end

        if played then
            method = "Note"
            message = "Elektronische Alarmsirene spielt"
        elseif #failures > 0 then
            message = table.concat(failures, " | ")
        end
    end

    recordAudioResult(state, alarmCue, played, method, message)

    if not played then
        if state.audio.lastCue ~= "Alarm" then
...
recordAudioResult = function(state, cue, ok, method, message)
...
local function playDfpwmCue(state, cue)
    ...
    while true do
        ...
        while true do
            local ok, result = speakerInvoke(state, "playAudio", buffer, CUSTOM_AUDIO_VOLUME)
            if ok and result ~= false then
                break
            end
...
local function playAudioCue(state, cue)
    ...
    if cue.kind == "sound" then
        local ok, result = speakerInvoke(state, "playSound", cue.sound, cue.volume or 3, cue.pitch or 1)
        if ok and result ~= false then
            return true, "Sound", cue.label .. " gespielt"
        end
        return false, "Sound", cue.label .. " blockiert: " .. formatSpeakerResult(ok, result)
    end
...
function runtime.isSecurityUnlocked(state)
    return state.crew and state.crew.session and state.crew.session.role ~= nil
end

function runtime.securityStatus(state)
    if state.crew and state.crew.session and state.crew.session.role then
        return "Per Rolle", state.theme.ok
    end

    return "Login offen", state.theme.muted
end

function runtime.securitySourceLabel()
    return "Keine Zusatzfreigabe"
end

function runtime.securitySummary()
    return "Kritische Funktionen folgen nur noch dem Rollen-Login"
end
...
function runtime.authorizeAction(state, action, context)
    ...
    if action == "toggle_alarm" and context.source ~= "remote" then
        return true
    end
...
function runtime.toggleManualAlarm(state)
    ...
    if state.manualAlarm and state.speaker then
        state.alarmFlash = true
        playAlarmTone(state)
        if state.audio then
            state.audio.alarmCooldown = 0
        end
...
function runtime.speakerStatus(state)
    if state.speaker then
        if state.assignments and state.assignments.alarmOutputSide ~= "none" and state.assignments.alarmOutputSide == state.speakerName then
            return "Verbunden / I/O gleich", state.theme.warning
        end
...
function runtime.audioLastMethod(state)
    return state.audio.lastMethod or "-"
end
...
function runtime.testSpeaker(state)
    ...
    state.audio.testTicks = 0
    state.audio.testPulse = 0
    ...
    local steps = {
        {
            method = "playSound",
            label = "Sound",
            args = { "block.bell.use", 3, 1.1 },
            successMessage = "Speaker-Test spielt",
        },
        { delay = 1 },
        {
            method = "playSound",
            label = "Sound",
            args = { "entity.creeper.primed", 3, 1.0 },
            successMessage = "Speaker-Test spielt",
        },
        { delay = 1 },
        {
    ...
    local lastMethod = "-"
    local lastMessage = "Speaker-Test ohne Audio"
    for _, step in ipairs(steps) do
        if step.delay then
            sleep(step.delay)
        else
            local ok, method, message = invokeSpeakerSequence(state, { step })
            if ok then
                playedAny = true
            end
            lastMethod = method or lastMethod
            lastMessage = message or lastMessage
            recordAudioResult(state, state.audio.lastCue and { label = state.audio.lastCue } or { label = "Speaker-Test" }, ok, lastMethod, lastMessage)
        end
    end

    runtime.setNotice(state, playedAny and "Speaker-Test gespielt" or "Speaker-Test ohne Audio", playedAny and 24 or 36)
    runtime.log(state, "Speaker-Test gestartet auf " .. runtime.speakerLabel(state), "ALARM")
    return playedAny, lastMessage
end
...
function runtime.debugLines(state)
    ...
    return {
        ...
        "Access: " .. tostring(securityText) .. " | User=" .. runtime.currentUsername(state) .. " | Tablet=" .. runtime.linkedTabletLabel(state),
        "Audio-Cue: " .. runtime.audioCueLabel(state) .. " | Custom: " .. tostring(runtime.customAudioCount(state)) .. " | Last: " .. runtime.audioLastMethod(state) .. ": " .. runtime.audioLastResult(state),
...
local function updateHelmAndFactory(state)
    ...
    elseif runtime.isSecurityUnlocked(state) then
        state.helm.failsafe = "Per Rolle"
    else
        state.helm.failsafe = "Login offen"
    end
...
function runtime.tick(state)
    ...
    if state.audio and state.audio.alarmCooldown and state.audio.alarmCooldown > 0 then
        state.audio.alarmCooldown = state.audio.alarmCooldown - 1
    end
    ...
    if state.audio and state.audio.testTicks and state.audio.testTicks > 0 then
        ...
        local ok, method, message = invokeSpeakerSequence(state, { tone })
        state.audio.lastResult = message or (ok and "Speaker-Test spielt" or "Speaker-Test blockiert oder stumm")
        state.audio.lastMethod = method or "-"
        state.audio.lastTime = nowStamp()
    ...
    if state.speaker and (state.audio.alarmCooldown or 0) <= 0 then
        local played = playAlarmTone(state)
        state.audio.alarmCooldown = played and 0 or 1
    end
end
