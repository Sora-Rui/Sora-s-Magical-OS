...
function runtime.testSpeaker(state)
    ...
    state.audio.testPulse = 2
    state.audio.lastCue = "Speaker-Test"

    local started, method, message = invokeSpeakerSequence(state, {
        {
            method = "playSound",
            label = "Sound",
    ...
    state.audio.lastResult = message or "Speaker-Test gestartet"
    state.audio.lastMethod = method or (state.speakerName or "auto")
    state.audio.lastTime = nowStamp()

    runtime.setNotice(state, started and "Speaker-Test gestartet" or "Speaker-Test ohne Audio", started and 24 or 36)
