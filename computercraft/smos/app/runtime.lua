...
local alarmNotes = {
    {
        siren = {
            { instrument = "bit", pitch = 8, volume = 3 },
            { instrument = "didgeridoo", pitch = 6, volume = 2 },
        },
        fallbackSound = "entity.creeper.primed",
        fallbackPitch = 0.8,
    },
    {
        siren = {
            { instrument = "bit", pitch = 12, volume = 3 },
            { instrument = "didgeridoo", pitch = 9, volume = 2 },
        },
        fallbackSound = "entity.creeper.primed",
        fallbackPitch = 0.95,
    },
    {
        siren = {
            { instrument = "bit", pitch = 16, volume = 3 },
            { instrument = "didgeridoo", pitch = 12, volume = 2 },
        },
        fallbackSound = "entity.creeper.primed",
        fallbackPitch = 1.1,
    },
    {
        siren = {
            { instrument = "bit", pitch = 20, volume = 3 },
            { instrument = "didgeridoo", pitch = 15, volume = 2 },
        },
        fallbackSound = "entity.creeper.primed",
        fallbackPitch = 1.25,
    },
...
local function playAlarmTone(state)
    ...
    if note and note.siren then
        ...
        if played then
            method = "Siren"
            message = "Alarmsirene spielt"
        elseif #failures > 0 then
            message = table.concat(failures, " | ")
        end
    end

    if not played and note and note.fallbackSound then
        local ok, result = speakerInvoke(state, "playSound", note.fallbackSound, 3, note.fallbackPitch or 1.0)
        if ok and result ~= false then
            played = true
            method = "Sound"
            message = "Alarmsirene spielt"
        else
            message = message .. " | Fallback: " .. formatSpeakerResult(ok, result)
        end
    end
...
function runtime.tick(state)
    ...
    if state.alarmFlash and (state.audio.alarmCooldown or 0) <= 0 then
