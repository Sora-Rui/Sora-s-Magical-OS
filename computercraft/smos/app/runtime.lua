...
local function playAlarmTone(state)
    ...
    local played = false
    if sound and state.speaker.playSound then
        local ok, result = pcall(state.speaker.playSound, sound.name, 3, sound.pitch)
        played = ok and result ~= false
    end

    if not played and state.speaker.playNote then
        local note = alarmNotes[state.alarmPulse]
        if note then
            pcall(state.speaker.playNote, note.instrument, 3, note.pitch)
        end
    end
...
local function playDfpwmCue(state, cue)
    if not state.speaker or not state.speaker.playAudio then
    ...
    while true do
        ...
        while not state.speaker.playAudio(buffer, 1.0) do
...
function runtime.testSpeaker(state)
    ...
    if state.speaker.stop then
        pcall(state.speaker.stop)
    end

    local played = false
    if state.speaker.playSound then
        local ok, result = pcall(state.speaker.playSound, "block.bell.use", 3, 1.1)
        played = ok and result ~= false
    end

    if not played and state.speaker.playNote then
        local ok, result = pcall(state.speaker.playNote, "pling", 3, 18)
        played = ok and result ~= false
    end

    if played then
        state.audio.lastCue = "Speaker-Test"
        state.audio.lastResult = "Speaker-Test gesendet"
        state.audio.lastMethod = state.speaker.playSound and "Sound" or "Note"
        state.audio.lastTime = nowStamp()
        runtime.setNotice(state, "Speaker-Test gesendet", 24)
        runtime.log(state, "Speaker-Test ausgegeben", "ALARM")
        return true
    end

    state.audio.lastCue = "Speaker-Test"
    state.audio.lastResult = "Speaker erkannt, aber Ton blockiert oder stumm"
    state.audio.lastMethod = "-"
    state.audio.lastTime = nowStamp()
    runtime.setNotice(state, "Speaker erkannt, aber Ton blockiert oder Sound stumm", 36)
    runtime.log(state, "Speaker-Test blockiert", "ALARM")
    return false, "Sound blockiert"
end
