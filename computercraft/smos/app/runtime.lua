...
local alarmNotes = {
    { sound = "entity.creeper.primed", soundPitch = 1.0, instrument = "pling", pitch = 18 },
    { sound = "block.bell.use", soundPitch = 1.05, instrument = "bell", pitch = 12 },
    { sound = "entity.creeper.primed", soundPitch = 0.85, instrument = "bit", pitch = 20 },
    { sound = "block.bell.use", soundPitch = 0.9, instrument = "bell", pitch = 8 },
}
...
local function playAlarmTone(state)
    ...
    local steps = {}
    if note and note.sound then
        steps[#steps + 1] = {
            method = "playSound",
            label = "Sound",
            args = { note.sound, 3, note.soundPitch or 1.0 },
            successMessage = "Alarmton spielt",
        }
    end
    if note then
        steps[#steps + 1] = {
            method = "playNote",
            label = "Note",
            args = { note.instrument, 3, note.pitch },
            successMessage = "Alarmton spielt",
        }
    end

    local played, method, message = invokeSpeakerSequence(state, steps)
...
function runtime.tick(state)
    ...
    if state.alarmFlash and (state.audio.alarmCooldown or 0) <= 0 then
        ...
        state.audio.alarmCooldown = played and 3 or 1
    end
end
