...
local function roleAllowsRemote(role, action)
    local roleInfo = ROLE_DEFINITIONS[role or ""]
    if not roleInfo then
        return false
    end

    return roleInfo.remoteActions[action] == true
end
...
local function saveConfig(state)
    return saveSerialized(CONFIG_PATH, {
        ...
        activeRole = state.crew.activeRole,
...
function runtime.newState(theme)
    ...
    local state = {
        ...
        crew = {
            activeRole = config.activeRole or "pilot",
        },
        security = {
...
function runtime.roleLabel(state)
    local info = ROLE_DEFINITIONS[state.crew.activeRole] or ROLE_DEFINITIONS.pilot
    return info.label
end
...
function runtime.setRole(state, roleName)
    if ROLE_DEFINITIONS[roleName] then
        state.crew.activeRole = roleName
        saveConfig(state)
        runtime.log(state, "Crew-Rolle -> " .. ROLE_DEFINITIONS[roleName].label, "CREW")
        runtime.setNotice(state, "Rolle: " .. ROLE_DEFINITIONS[roleName].label, 24)
        return true
    end
...
function runtime.authorizeAction(state, action, context)
    ...
    if context.source == "remote" and not roleAllowsRemote(context.role, action) and action ~= "request_status" then
        return false, "Rolle darf das nicht"
    end
