...
function ui.monitorFrame(title, subtitle, state)
    local width, height = term.getSize()
    ui.frame(title, "Touch aktiv  |  Home links oben  |  Alarm mit M", state)
    ...
    ui.writeAt(4, 6, "Monitor-Deck aktiv", theme.muted, theme.panel)
    ui.writeAt(width - 22, 6, state.monitor and "Monitor verbunden" or "Lokaler Rechner", state.monitor and theme.ok or theme.warning, theme.panel)
end
...
function ui.navTabs(state, y, activeScreen)
    local tabs = {
        { label = "Steuerung", action = "helm" },
    ...
    for _, tab in ipairs(tabs) do
        ui.button(state, x, y, #tab.label + 3, tab.label, tab.action, activeScreen == tab.action)
        x = x + #tab.label + 6
    end
end
