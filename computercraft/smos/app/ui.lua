...
function ui.monitorFrame(title, subtitle, state)
    ...
    ui.frame(title, "Touch aktiv  |  U PIN  |  M Alarm  |  Q Ende", state)
...
function ui.warningOverlay(title, lines, blinkOn)
    ...
    if os and os.clock then
        pulseOn = math.floor(os.clock() * 4) % 2 == 0
    end

    local background = pulseOn and theme.warning or theme.accentLight
    local titleColor = pulseOn and theme.text or theme.shadow
